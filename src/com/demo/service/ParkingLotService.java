...
import com.demo.repo.ParkingDataRepository;
...
public class ParkingLotService {
    ...
    public void unParkVehicle(String ticketId) {
        if(vehicleHashMap.get(ticketId)!=null){
            ...
            parkingLot.getParkingFloors().get(parkingSlot.getFloorId()).getParkingSlots().get(parkingSlot.getSlotId()).setEmpty(true);
            vehicleHashMap.remove(ticketId);
            System.out.println("Unparked vehicle with Registration Number:"+vehicle.getRegistrationId()+
                    "and Color:"+ vehicle.getColour());
        }
        else
    ...
    private ParkingSlot getFirstAvailableSlot(VehicleType vehicleType) {
        ...
        for(ParkingFloor parkingFloor: parkingFloors){
            ...
            for(ParkingSlot parkingSlot:parkingSlots) {
                if(parkingSlot.getVehicleType().equals(vehicleType) && parkingSlot.isEmpty()){
                    return parkingSlot;
                }
            }
        }
    ...
    private void displayFreeSlots(VehicleType vehicleType){
        for(int i=0; i< parkingLot.getParkingFloors().size();i++) {
            ...
            List<ParkingSlot> collect = parkingFloor.getParkingSlots()
                    .stream()
                    .filter(parkingSlot -> parkingSlot.getVehicleType().equals(vehicleType))
                    .filter(ParkingSlot::isEmpty).collect(Collectors.toList());
    ...
    private void displayOccupiedSlots(VehicleType vehicleType){
        for(int i=0; i< parkingLot.getParkingFloors().size();i++) {
            ...
            List<ParkingSlot> collect = parkingFloor.getParkingSlots()
                    .stream()
                    .filter(parkingSlot -> parkingSlot.getVehicleType().equals(vehicleType))
                    .filter(parkingSlot -> !parkingSlot.isEmpty()).collect(Collectors.toList());
